...
SELECT * FROM orders WHERE date::TEXT LIKE '2025-03%';

 9. `orders` 테이블에서 2025년 3월 12일 오전에 주문된 내역만 조회하세요.
SELECT * FROM odeers WHERE date = '2025-03-12' AND time < '12:00:00';
...
SELECT date, COUNT(order_id) AS total_orders, SUM(total_price) AS total_amount
FROM orders
...
SELECT
p.name AS P.pizza_name,
P.size AS P.pizza_size,
P.price AS P.pizza_price,
O.quantity AS P.quantity,
...
SELECT P.size, SUM(O.quantity * P.price) AS total_revenue
FROM order_details O
